...
var KisBpmChoseAssignmentCtrl = ['$scope', '$http', function($scope, $http) {
    ...
    $scope.getAllRoles = function (successCallback) {
        $http({
            ...
            url: '/system/role/allRole'}).success(function (data, status, headers, config) {
                var obj = data;
                for (var i=0; i<obj.length; i++) {
                    ...
                    roles.push({id:obj[i].roleId,name:obj[i].roleName});
                }
                $scope.roles = roles;
    ...
    $scope.getAllAccountByRole = function(value) {
        $http({
            method: 'get',
            headers: {'Accept': 'application/json', 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
            url: '/system/role/users/'+value}).success(function (data, status, headers, config) {
                //封装数据
                var obj = data;
                if (data != null) {
                    var accounts = [];
                    for (var i=0; i<obj.length; i++) {
                        accounts.push({id:obj[i].userId, code : obj[i].loginName, name : obj[i].userName, index:i});
                    }
    ...
    $scope.getAllRoles = function (successCallback) {
        $http({
            ...
            url: '/system/role/allRole'}).success(function (data, status, headers, config) {
                var obj = data;
                for (var i=0; i<obj.length; i++) {
                    candidateGroups.push({id:obj[i].roleId,name:obj[i].roleName,description:obj[i].remark});
                }
